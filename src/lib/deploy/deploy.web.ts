...
export class DeploymentsManager extends BaseDeploymentsManager {
  ...
  async deployContract({
    ...
    const walletInstance = await Wallet.load(wallet as JWKInterface);
    const owner = await walletInstance.getAddress();
    const signer = createDataItemSigner(
      walletInstance.jwk || window.arweaveWallet
    );
    ...
    logActionStatus("deploy", logger, "provided contract source", blueprints);
